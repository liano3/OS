...
MODULE_VERSION("1.0");
// 定义要输出的文件
#define OUTPUT_FILE "/home/ningli/oslab/lab3/part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff8e5092a0;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff8e4ea8a0;
...
static unsigned int ktest_func = 4;
...
static void flush_buf(int end_flag)
{
    ...
    if (end_flag == 1)
    {
        if (likely(curr_buf_length > 0)) // likely 表示大概率为 true
...
static void scan_vma(void)
{
    ...
    if (mm){
        ...
        // 此函数用于减少任务对应的内存空间的用户的数量，并在当用户数量减少到0时释放任务所占用的内存空间。
        mmput(mm);
    }
}
