    ...
    while (**remains != '\0' && state == FIND_EXIST) {
        ...
        if(level == 0) {
            ...
            while (is_cluster_inuse(clus)) {    // 依次查找每个簇
                ...
                // 计算第一个扇区号
                sector_t from_sector = cluster_first_sector(clus);
                // 查找目录项
                state = find_entry_in_sectors(*remains, len, from_sector, meta.sec_per_clus, slot);
...
int fat16_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, 
    ...
    while (root || is_cluster_inuse(clus)) {
        ...
        // filler 的使用方法： filler(buf, 文件名, NULL, 0, 0) 。。。少个 0
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    for(size_t i = 0; i < meta.fats; i++) {
        ...
        //   1. 计算第 i 个 FAT 表所在扇区，进一步计算clus对应的FAT表项所在扇区
        //   2. 读取该扇区并在对应位置写入数据
        //   3. 将该扇区写回
...
int alloc_clusters(size_t n, cluster_t* first_clus) {
    ...
    for(size_t i = 0; i < n; i++) {
        int ret = cluster_clear(clusters[i]);   // 请实现cluster_clear()  // ? 不是实现好了
        if(ret < 0) {
            free(clusters);
            return ret;
        }
...
int fat16_mknod(const char *path, mode_t mode, dev_t dev) {
    ...
    ret = dir_entry_create(slot, shortname, ATTR_REGULAR, 0xFFFF, 0);
    if(ret < 0) {
        return ret;
    }
    return 0;
